...
CREATE PROCEDURE usp_makeReservation @customerId INT 
AS
BEGIN
-- This section validates user input before making a reservation
    ...
    IF @name IS NULL
    BEGIN
		THROW, 5001, 'Please enter name.', 1;

    IF @address IS NULL
    BEGIN
		THROW, 5002, 'Please enter address.', 1;
	
	IF @phoneNumber IS NULL
    BEGIN
		THROW, 5003, 'Please enter phone number.', 1;
	
    IF @email IS NULL
    BEGIN
		THROW, 5004, 'Please enter email.', 1;
